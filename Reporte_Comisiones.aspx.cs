    ...
    public partial class Reporte_Comisiones : System.Web.UI.Page
    {
        ...
        protected void BTN_FILTRAR_FECHAS_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    SqlDataAdapter da = new SqlDataAdapter("SP_REPORTE_COMISION_X_FECHA", conexion);
                    ...
                    da.SelectCommand.Parameters.AddWithValue("@ID_USUARIO", int.Parse(Request.Cookies["ID_USUARIO"].Value.ToString()));
                    da.Fill(dt);
        ...
        protected void BTN_DESCARGAR_CSV_ServerClick(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection conexion = new SqlConnection(ConfigurationManager.ConnectionStrings["Operaciones.Properties.Settings.CONEXION"].ToString()))
                {
                    ...
                    sb.Append("% COMISION" + delimitador);
                    ...
                    foreach (DataRow data in dt.Tables["REPORTES"].Rows)
                    {
                        ...
                        sb.Append(data["NOMBRE_COMERCIAL"].ToString() + delimitador);
                        sb.Append(data["PORCENTAJE"].ToString() + delimitador);
